...
| `compute.history.retentionDays` | int | `7` | Days of history kept. `0` disables history inserts and pruning; the live table still works. |
...
The two `POST`s are for the controller and require the bearer token when
`broker.auth.enabled` is set.
...
| `GET /compute/findings?namespace=<ns>&node=<node>` | `{ "findings": [Finding] }`. Both filters optional; neither means the whole cluster. |
...
| `culprit` | `null` for `cpu-throttled`, `cpu-contended` and `memory-limit-thrash`. `kind` is `pod`, `system` or `kernel`; the `pod_*` fields are `null` unless `kind` is `pod`. `blame_share` is the culprit's share of the victim's wait time over the window; `cpu_usage_millis` and `cpu_request_millis` are the numbers behind "running beyond its request" (`cpu_request_millis` is `null` when the culprit has none). |
