...
<Steps>
  ...
  <Step title="Micro bar">
    ...
    80 % of a node are different situations. A multi-container pod is drawn
    from its pod-level cgroup; the worst container drives the dot.
  </Step>
  ...
  <Step title="Findings">
    ...
    *Generate policy* action, a compute finding offers **Resources**, which
    takes you to the victim's or the culprit's workload. Nothing is applied.
  </Step>
</Steps>

A node whose pods have no gauge at all is one of three things, and the node
tooltip says which: the feature is off on that node, the node is cgroup v1 or
has no PSI (unsupported), or its controller predates the feature.
