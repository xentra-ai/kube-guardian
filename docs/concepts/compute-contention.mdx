...
behaviour, not hidden. The broker does not filter findings by namespace
policy: a namespace in `controller.excludedNamespaces` never registers with
the controller, so its pods are never sampled and never appear as victims.
They can still appear as culprits, but only as an untracked `pod:<uid8>`
