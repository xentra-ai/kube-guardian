...
| `memory-pressure` | the container's `memory.pressure some avg10` ≥ `memStallSome` (default 10 %) or its refault rate is rising, **while** the node itself is under memory pressure (`/proc/pressure/memory some` ≥ 5 %) | the container on the node with the largest growth beyond its memory request, if it accounts for ≥ 40 % of the node's overage; labelled as a heuristic | set or raise memory requests on the culprit; add memory to the node |
| `memory-limit-thrash` | the container is refaulting under its **own** `memory.max` (`memory.events high` rising) and the node is not under pressure | never | raise `resources.limits.memory` on the victim |
...
| `kind` | `ref` | Meaning |
|---|---|---|
| `pod` | `namespace/pod/container` | Another tracked container on the node. Carries `pod_uid`, `namespace`, `pod_name` so the UI can draw an edge to it. A container kguardian does not track (excluded namespace, or not registered yet) is shown as `pod:<uid8>`. |
...
the window — and the finding is labelled as such. Treat it as a strong lead,
not a verdict.
...
behaviour, not hidden. The findings endpoint honours the same namespace
exclusions as traffic (`controller.excludedNamespaces`).
...
- **cgroup v1 nodes are not supported.** Per-cgroup pressure files do not
  ...
  are drawn.
- **The scheduler probe attaches via `tp_btf` and falls back to `raw_tp`.**
  If neither attaches, the node reports `contention_loaded: false` and keeps
  its gauges and stall findings; it just never produces a `noisy-neighbor`.
- **A node with no compute row at all** is either running with
  `compute.enabled=false` or a controller older than this feature.
