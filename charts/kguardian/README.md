...
| broker.initContainer.image.tag | string | `"1.37.0"` | Broker init container image tag, pinned rather than `latest`. |
...
| controller.initContainer.image.tag | string | `"1.37.0"` | Init container image tag. Pinned rather than `latest`: every other image in this chart is pinned, an unpinned tag means whatever it resolves to at pull time runs on every node, and `latest` is what forces `pullPolicy: Always` to re-check. |
