...
| compute.history.retentionDays | int | `7` | Days of per-pod compute and contention history the broker keeps. Minute rows are downsampled to 5-minute rows after `minuteResolutionHours`, so 7 days at 3 000 containers stays under ~10 M rows. `0` disables history inserts and pruning; the live gauges (`pod_compute_latest`) still work. Rendered into `COMPUTE_HISTORY_RETENTION_DAYS`. |
| compute.sampleInterval | int | `5` | Seconds between cgroup samples. Drives the live gauges and the `pod_compute_latest` upsert; the controller folds twelve samples into one minute row for history regardless of this value. |
| compute.thresholds | object | `{"blameShare":0.4,"memStallSome":10,"runqP99Ms":20,"stallSome":20,"throttledRatio":0.25,"throttledRatioMax":0.1}` | Finding thresholds, evaluated by the broker over the last five minutes of history. Rendered into `COMPUTE_THRESHOLD_*` on the broker. |
