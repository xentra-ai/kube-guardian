...
use aya_ebpf::{
    cty::c_long,
    helpers::{bpf_get_current_task, bpf_probe_read, bpf_probe_read_kernel},
    macros::{map, tracepoint},
    maps::{HashMap, PerfEventArray},
    programs::TracePointContext,
};

use kube_guardian_common::{SysCallLog, TrafficLog, BPF_MAPS_CAPACITY};
...
use bindings::{
    iphdr, net_device, ns_common, nsproxy, pid_namespace, sk_buff, task_struct, tcphdr, udphdr,
};

#[map]
pub static NETWORK_EVENTS: PerfEventArray<TrafficLog> = PerfEventArray::new(0);
...
pub fn kube_guardian_syscalls(ctx: TracePointContext) -> u32 {
    match unsafe { try_kube_guardian_syscalls(ctx) } {
        Ok(ret) => ret as u32,
        Err(ret) => ret as u32,
    }
}
...
unsafe fn try_kube_guardian_syscalls(ctx: TracePointContext) -> Result<c_long, c_long> {
    let task: TaskStructPtr = bpf_get_current_task() as TaskStructPtr;
    let syscall_nbr: u32 = unsafe { ctx.read_at(8)? };
    if !syscall_nbr.eq(&2) && !syscall_nbr.eq(&202) && !syscall_nbr.eq(&281)  && !syscall_nbr.eq(&0) &&  !syscall_nbr.eq(&1) && !syscall_nbr.eq(&232) {
    ...
    let log_entry = SysCallLog { inum , syscall_nbr};
...
unsafe fn try_kube_guardian(ctx: TracePointContext, traffic_type: i32) -> Result<c_long, c_long> {
    ...
    let dev_ptr = bpf_probe_read(
        &(*tp).__bindgen_anon_1.__bindgen_anon_1.__bindgen_anon_1.dev as *const *mut net_device,
    )
    .map_err(|_| 100u32)?;
    ...
    if if_index_map.eq(&1) {
        let eth_proto =
            bpf_probe_read(&(*tp).__bindgen_anon_5.headers.as_ref().protocol as *const u16)
                .map_err(|_| 100u32)?;
        ...
        let network_header_offset = bpf_probe_read(
            &(*tp)
                .__bindgen_anon_5
                .__bindgen_anon_1
                .as_ref()
                .network_header as *const u16,
        )
        .map_err(|_| 100u16)?;
...
unsafe fn get_transport_header_ptr(tp: *const sk_buff, head: *const u8) -> Result<*const u8, u16> {
    let transport_header_offset =
        bpf_probe_read(&(*tp).__bindgen_anon_5.headers.as_ref().transport_header as *const u16)
            .map_err(|_| 100u16)?;
